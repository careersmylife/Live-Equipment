...
const App: React.FC = () => {
  ...
  const handleAddAlert = (alertData: Omit<CustomAlert, 'id' | 'isEnabled' | 'isTriggeredFor'>) => {
    // FIX: Use a ternary to properly narrow the discriminated union before spreading.
    // This prevents properties specific to one union member (like 'condition') from becoming optional.
    const newAlert: CustomAlert =
      alertData.metric === 'health' ? {
        ...alertData,
        id: `alert-${Date.now()}`,
        isEnabled: true,
        isTriggeredFor: [],
